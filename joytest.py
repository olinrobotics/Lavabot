...
joystick = {'arm': 2, 'disarm': 3, 'failsafe': 0, 'auto': 4, 'manual': 5, 'send_waypoint': 6, 'x': 0, 'y': 1, 'z': 3, 'yaw': 2}
xbox     = {'arm': 0, 'disarm': 1, 'failsafe': 8, 'auto': 2, 'manual': 3, 'send_waypoint': 6, 'x': 3, 'y': 4, 'z': 1, 'yaw': 0}
rcsim    = {'arm': 2, 'disarm': 2, 'failsafe': 8, 'auto': 1, 'manual': 3, 'send_waypoint': 6, 'x': 0, 'y': 1, 'z': 2, 'yaw': 4}
...
class Joystick():
	...
	self.pub_waypoint = rospy.Publisher('/send_waypoint', Bool, queue_size=10)
    ...
    def run(self):
        ...
        # send waypoint button handler
        waypoint_data = self.buttons[ctrl['send_waypoint']]
        self.waypoint, self.last_waypoint = not self.last_waypoint and waypoint_data, waypoint_data
	if self.waypoint:
		self.pub_waypoint.publish(Bool)
